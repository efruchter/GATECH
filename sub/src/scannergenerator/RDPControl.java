...
public class RDPControl {
    ...
    public String generateRegex(String filename){
		...
		while (controller.classes[index2] != null) {
			controller.classes[index2].setRegex(parseFinal(new String(controller.classes[index2].getRegex())).toCharArray());
			index2++;
		}
		DefinedClass[] copyofrange = Arrays.copyOfRange(controller.classes,tokenindex,controller.classes.length-1);
    ...
    private String mergeDefinedClasses(DefinedClass[] classes){
        ...
        for (DefinedClass c : classes){
            if (c == null) return s.substring(0,s.length() -1 );
